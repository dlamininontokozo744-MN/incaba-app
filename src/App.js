  ...
  return (
    <div style={{background:'rgba(201,162,39,0.06)',border:'0.5px solid rgba(201,162,39,0.25)',borderRadius:14,padding:14,marginBottom:16}}>
      ...
      <div style={{display:'flex',gap:10,alignItems:'center',marginBottom:12}}>
        <input type="number" value={amount} onChange={e=>setAmount(e.target.value)} style={{flex:1,background:'rgba(255,255,255,0.06)',border:'0.5px solid rgba(201,162,39,0.3)',borderRadius:10,padding:'10px 14px',color:'#f0f4ff',fontSize:16,fontWeight:700,outline:'none'}} placeholder="Amount"/>
    ...
    <div style={styles.app}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:16}}>
        ...
        <div style={{display:'flex',gap:10,marginTop:8,marginBottom:8}}>
          <button style={{...styles.btnPrimary,flex:1,padding:'12px',fontSize:14}} onClick={()=>window.open(`https://www.google.com/maps/search/${encodeURIComponent(place.name)}+Eswatini`,'_blank')}>🗺️ {t.getDir}</button>
  ...
  return (
    <div style={styles.app}>
      <div style={{background:'linear-gradient(135deg,#1a3a1a,#2d5a2d)',height:180,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,position:'relative'}}>
        ...
        {cart.length>0&&<button onClick={()=>setShowCart(!showCart)} style={{position:'absolute',top:16,right:16,background:'rgba(201,162,39,0.9)',border:'none',borderRadius:50,padding:'8px 14px',color:'#0a1628',fontSize:13,fontWeight:700,cursor:'pointer'}}>🛒 {cart.length} · E{total}</button>}
      </div>
      <div style={{flex:1,overflowY:'auto',padding:16}}>
          ...
          <div key={cat.category} style={{marginBottom:16}}>
            ...
            {cat.items.map(menuItem=>(
              <div key={menuItem.name} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:'0.5px solid rgba(255,255,255,0.04)'}}>
                ...
                <button onClick={()=>addToCart(menuItem)} style={{padding:'8px 14px',borderRadius:50,background:'linear-gradient(135deg,#c9a227,#e8b93a)',border:'none',color:'#0a1628',fontSize:12,fontWeight:700,cursor:'pointer',flexShrink:0}}>+ Add</button>
              </div>
            ))}
          </div>
        ))}
...
function MapTab({t}) {
  ...
  return (
    <div>
      ...
      {routes.map(r=>(
        <div key={r.name}>
          <div onClick={()=>setActiveRoute(activeRoute===r.name?null:r.name)} style={{background:'rgba(255,255,255,0.05)',border:`0.5px solid ${activeRoute===r.name?r.color:'rgba(201,162,39,0.2)'}`,borderRadius:activeRoute===r.name?'12px 12px 0 0':12,padding:'12px 14px',marginBottom:activeRoute===r.name?0:10,cursor:'pointer'}}>
      ...
      {step==='payment'&&(
        <div>
          ...
          <div style={{background:'rgba(201,162,39,0.08)',border:'0.5px solid rgba(201,162,39,0.25)',borderRadius:14,padding:16,marginBottom:20}}>
            ...
            <div style={{fontSize:26,fontWeight:700,color:'#c9a227'}}>E200.00</div>
